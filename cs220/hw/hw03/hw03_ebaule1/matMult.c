...
/* 
----------------------------
Math for Populating Matrix C
----------------------------
*/
void setC_Prod(int ra,int carb, int cb){
	int i; //temp variable to do math with
	int j;
	int k;
	//iterate through c first to index where we are putting t
	for(i = 0; i < ra; i++){
		for(j = 0; j < cb; j++){
			int t = 0; //make sure t is 0 before we do math
			for(k = 0; k < carb; k++){
				t += (matA[i][k]) * (matB[k][j]);
			}
			matC[i][j] = t;
